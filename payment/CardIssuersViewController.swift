            ...
            if let parsedObjects = parsedObjects {
                ...
                if self != nil {
                    DispatchQueue.main.async() {
                        let labelWidth = self!.tableView.bounds.size.width
                        let labelHeight = self!.tableView.bounds.size.height
                        let emptyDataLabel = UILabel(frame: CGRect(x: 0, y: 0, width: labelWidth, height: labelHeight))
                        emptyDataLabel.lineBreakMode = .byWordWrapping
                        emptyDataLabel.numberOfLines = 0
                        emptyDataLabel.text = "No hay tarjetas disponibles para este banco o medio de pago."
